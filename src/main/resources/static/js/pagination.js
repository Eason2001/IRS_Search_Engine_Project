jQuery.fn.pagination = function(maxentries, opts) {
	opts = jQuery.extend({
				...
				num_edge_entries : 4, // 末尾显示页码的个数
				link_to : "result?queryString="+ $("#qString").attr("value") + "&" + "pageIndex=",   //页码点击后的链接
	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			if (opts.display_msg) {
				if(!maxentries){
					panel
						.append('<div class="pxofy">No items to display!</div>');
				}else{
				panel
						.append('<div class="pxofy">Show&nbsp;'
								+ ((current_page * opts.items_per_page) + 1)
								+ '&nbsp;to&nbsp;'
								+ (((current_page + 1) * opts.items_per_page) > maxentries
										? maxentries
										: ((current_page + 1) * opts.items_per_page))
								+ '&nbsp;，total&nbsp;' + maxentries + '&nbsp;items</div>');
				}
			}
			//设置跳到第几页
			if(opts.setPageNo){
				  panel.append("<div class='goto'><span class='text'>go to</span><input type='text'/><span class='page'>page</span><button type='button' class='ue-button long2'>confirm</button></div>");	
			}
		}
		...
		if(maxentries==0){
			panel.append('<span class="current prev">'+opts.prev_text+'</span><span class="current next">'+opts.next_text+'</span><div class="pxofy">No items to display!</div>');
		}else{
			drawLinks();
		}
